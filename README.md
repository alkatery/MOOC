...
| الباقة | الشهري | السنوي | المقررات | الطلاب | الميزات الأبرز |
|------|------|------|------|------|------|
| تجريبي | مجاني | مجاني | 2 | 50 | شهادات، xAPI/SCORM |
| قياسي | 499 ر.س | 4,791 ر.س | 50 | 1,000 | + مساعد ذكي + تخصيص الهوية |
| متميّز | 1,999 ر.س | 19,191 ر.س | بلا حد | بلا حد | + نطاق مخصص + SSO + اعتماد NELC |

**ما يحصل عليه كل مشترك**:
- 🎨 **هوية بصرية**: شعار، أيقونة تبويب، لون رئيسي/تمييز/خلفية/نص.
- 🔗 **حسابات التواصل**: تويتر/X، لينكد إن، إنستغرام، يوتيوب، فيسبوك، تيك توك، سناب شات، موقع.
- 📜 **سياسات مخصصة**: خصوصية، شروط، استرداد، ميثاق سلوك.
- 🏷 **تخصيص المسميات**: استبدل "مقرر" بـ "حقيبة"، "طالب" بـ "متدرب"… دون تعديل كود.
- 📄 **صفحات مخصصة (CMS)**: عن المنصة، الأسئلة الشائعة، شركاؤنا… تظهر تحت `/p/<slug>`.
- 💳 **باقة وفوترة**: ترقية فورية بين الباقات شهرياً أو سنوياً.

**كيف يعمل**:
- تصفح الباقات: `/pricing`
- إنشاء منصة جديدة (١٤ يوم تجربة): `/signup`
- لوحة المشترك: `/tenant/settings` و `/tenant/pages` و `/tenant/billing`
- توجيه المنصات: عبر النطاق الفرعي (مثلاً `acme.mooc.sa`) أو الكوكي `mooc_tenant` أو `?t=<slug>`.

النواة في `mooc/services/tenants.py` (resolution) و `mooc/routers/saas.py` (إدارة).

## المساعد الذكي · AI assistants

تتضمن المنصة مساعدَيْن مبنيين على **نموذج محلي عبر [Ollama](https://ollama.com)**
لضمان بقاء بيانات المتعلم داخل حدود إقامة البيانات السعودية:

- **مساعد الطالب** — متاح من صفحة الدرس (`/student/lesson/...`) وصفحة
  الواجب (`/student/.../assignment/...`):
  - يجيب على أسئلة الطالب بناءً على محتوى الدرس فقط (RAG محلي).
  - يعطي *تلميحات* للواجبات تقود الطالب إلى الدرس الذي يحتوي الإجابة
    دون كتابة الإجابة (التزاماً بسياسة النزاهة الأكاديمية M.19).
- **تحليلات الإدارة الذكية** — `/admin/insights`:
  - الطلاب المتميّزون والمتعثّرون.
  - نقاط الاختناق: دروس يبدأها كثيرون ولا يكملونها.
  - أسئلة كثيرة الخطأ تحتاج مراجعة الصياغة.
  - ملخص إداري عربي يولّده النموذج المحلي بضغطة زر.

التشغيل:

```bash
# 1) تثبيت Ollama وتنزيل نموذج عربي قادر
curl -fsSL https://ollama.com/install.sh | sh
ollama pull llama3.1:8b-instruct

# 2) ضبط .env (اختياري — هذه القيم افتراضية)
echo "MOOC_OLLAMA_URL=http://localhost:11434" >> .env
echo "MOOC_OLLAMA_MODEL=llama3.1:8b-instruct" >> .env
```

إذا لم يكن Ollama قيد التشغيل، تعرض المنصة رسالة عربية لطيفة بدلاً من
الفشل، فتبقى قابلة للاستخدام بدون المساعد.

## متطلبات نظام الدورات (NELC) · NELC course-system requirements

تتبنى المنصة التصنيف الرسمي للمتطلبات التي يضعها المركز الوطني للتعليم
الإلكتروني لأنظمة المقررات إلى ثلاثة مستويات:

| المستوى | العدد | الوصف |
|---------|------|--------|
| إلزامي (Mandatory) | 30 | شرط لاعتماد المقرر — لا يمكن إرسال المقرر للمراجعة قبل اكتمالها |
| اختياري (Optional) | 20 | تحسينات تعزز جودة المقرر (ترجمة بلغة الإشارة، تقييم الأقران…) |
| تميز (Excellence) | 15 | متطلبات عليا تمنح المقرر تصنيف "متميز" (AR/VR، مختبرات افتراضية…) |

كل متطلب مرتبط بأحد معايير K-12 الأصلية (`K.x.y.z`) ويحمل دالة فحص
تلقائي عندما يكون الفحص ممكناً برمجياً (مثلاً وجود ترجمة لجميع
الفيديوهات أو منتدى نقاش)، أو يُعلَّم "يدوي" فيرفع المعلم دليلاً للمراجع.

- صفحة عامة: `/nelc/course-requirements`
- تقرير امتثال لكل مقرر يَعرضه المعلم: `/teacher/courses/{id}/requirements`
- تنفيذ: `mooc.nelc.course_requirements` و `mooc.services.course_requirements`
- عند إرسال مقرر للمراجعة، تُرفض العملية تلقائياً إن كان أي متطلب إلزامي غير مكتمل
